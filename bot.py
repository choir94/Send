...
def print_header():
    ...
    print(Fore.CYAN + Style.BRIGHT + "Auto Send Native Token / ERC-20 Token".center(50))
...
# Fungsi untuk mentransfer token ERC-20
def transfer_erc20(sender, senderkey, recipient, amount, web3, token_address, retries=3):
    for attempt in range(retries):
        try:
            # ABI standar untuk kontrak token ERC-20
            abi = [
                {
                    "constant": False,
                    "inputs": [
            ...
            # Membuat kontrak token
            contract = web3.eth.contract(address=token_address, abi=abi)

            # Menentukan jumlah token yang akan dikirim (dalam wei)
            token_amount = web3.to_wei(amount, 'ether')  # Gunakan 'ether' atau 'wei' sesuai token's decimals

            # Mempersiapkan transaksi
            tx = contract.functions.transfer(recipient, token_amount).buildTransaction({
                'chainId': web3.eth.chain_id,
                'from': sender,
                'gas': 200000,  # Atur gas sesuai dengan estimasi
                'gasPrice': web3.eth.gas_price,
                'nonce': web3.eth.get_transaction_count(sender)
            })
            ...
            print(Fore.CYAN + f'Processing send {amount} tokens to {recipient} ...')
            ...
            print(Fore.GREEN + f'Send {amount} tokens to {recipient} Success!')
            print(Fore.GREEN + f'TX-ID : {txid}')
...
# Ask the user for token address (optional, empty for native token)
token_address = input("Enter the ERC-20 token contract address (leave empty for native token): ")
...
for i in range(loop):
    ...
    if token_address:
        # Transfer token ERC-20 if token_address is provided
        transfer_erc20(sender.address, private_key, recipient.address, amount, web3, token_address)
    else:
        # Transfer native token if token_address is not provided
